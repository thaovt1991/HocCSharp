...
namespace HashTableDemo
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if(ht.ContainsKey("Dept"))
            {
                Console.WriteLine($"ton tai 'Dept' trong  {nameof(ht)}");
            }
            else
            {
                Console.WriteLine($" khong ton tai 'Dept' trong  {nameof(ht)}");
            }

            if (ht.ContainsKey(15))
            {
                Console.WriteLine($"ton tai '15' trong  {nameof(ht)}");
                Console.WriteLine("Gia tri tai key la  " + ht[15]);
            }
            else
            {
                Console.WriteLine($" khong ton tai '15' trong  {nameof(ht)}");
            }
            if (ht.ContainsValue(7))
            {
               foreach(var key in ht.Keys)
                {
                    if (7.Equals(ht[key]))
                    {
                        Console.WriteLine("key la "+ key);
                        break;
                    }
